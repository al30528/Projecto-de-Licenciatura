...
def dataset_roots(selection: str) -> list[tuple[str, Path]]:
    datasets = [("desktop", ROOT_DIR)]
    mobile_root = ROOT_DIR / "app movel"
    if mobile_root.exists():
        datasets.append(("mobile", mobile_root))
...
def validate_graph(dataset: str, root: Path, issues: list[Issue]) -> None:
    nav.configure_paths(root)
    try:
        graph, _floor_graphs = nav.build_campus_graph()
    ...
    selectable_nodes = nav.collect_selectable_nodes(graph)
...
def validate_critical_routes(dataset: str, graph, nodes: list[nav.SelectableNode], issues: list[Issue]) -> None:
    ...
    for profile, origin_building, origin_floor, origin_label, dest_building, dest_floor, dest_label, reduced in tests:
        ...
        path, distance = nav.calculate_path(graph, origin.node_id, destination.node_id, reduced)
        if not path:
...
def validate_dataset(name: str, root: Path, strict_way_tags: bool) -> list[Issue]:
    ...
    validate_graph(name, root, issues)
    return issues
